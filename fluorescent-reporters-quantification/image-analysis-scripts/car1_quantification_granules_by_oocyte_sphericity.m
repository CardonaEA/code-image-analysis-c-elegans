...
% % No out of focus slides
% blocks_processing.is_out_focus = 1;
% blocks_processing.is_half = 1;
...
for i = 1 : length(selected_files.GFP_image)
    info_image_to_analyze.image = [selected_files.Folder '/' selected_files.GFP_image{i}];
    info_image_to_analyze.outlines = [selected_files.Folder '/' selected_files.cell_outlines{i}];
...
if i>1
    writetable(T_merge_granules, [selected_files.Folder '/Analysis_GFP_granules_MERGED' '.csv'])
end
...
for j = 1 : length(selected_files.GFP_image)
    info_image_to_analyze.image = [selected_files.Folder '/' selected_files.GFP_image{j}];
    info_image_to_analyze.outlines = [selected_files.Folder '/' selected_files.cell_outlines{j}];
...
if j>1
    writetable(T_merge, [selected_files.Folder '/Analysis_GFP_local_conc_levels_MERGED' '.csv'])
end
