...
%     T = table(Info_TS(:,1),Info_TS(:,2),Info_TS(:,3),Info_TS(:,4),Info_TS(:,5),Info_TS(:,6),Info_TS(:,7),Info_TS(:,8),...
%         Info_TS(:,9),Info_TS(:,10),Info_TS(:,11),Info_TS(:,12),Info_TS(:,13),Info_TS(:,14),Info_TS(:,15),Info_TS(:,16),...
%         'VariableNames',{'img','cell','CAR1_total','CAR1_cond','CAR_dil','vol_cell','vol_cond','vol_dil',...
%         'img_filt','cell_filt','CAR1_total_filt','CAR1_cond_filt','CAR_dil_filt','vol_cell_filt','vol_cond_filt','vol_dil_filt'});
    T = table(repmat({image_info.name(1:end-4)},[size(Info_TS,1),1]),...
        ...
        'img_filt','cell_filt','CAR1_total_filt','CAR1_cond_filt','CAR_dil_filt','vol_cell_filt','vol_cond_filt','vol_dil_filt'});
    % save table
    writetable(T, [image_info.fold '/' image_info.name(1:end-4) '_GFP_local_levels_quant_filt' '.csv'])
else
    Info_TS = TS_i;
%     T = table(Info_TS(:,1),Info_TS(:,2),Info_TS(:,3),Info_TS(:,4),Info_TS(:,5),Info_TS(:,6),Info_TS(:,7),Info_TS(:,8),...
%         'VariableNames',{'img','cell','CAR1_total','CAR1_cond','CAR_dil','vol_cell','vol_cond','vol_dil'});
    T = table(repmat({image_info.name(1:end-4)},[size(Info_TS,1),1]),...
        ...
        'VariableNames',{'image_ID_name','img','cell','CAR1_total','CAR1_cond','CAR_dil','vol_cell','vol_cond','vol_dil'});
    % save table
    writetable(T, [image_info.fold '/' image_info.name(1:end-4) '_GFP_local_levels_quant' '.csv'])
end
end