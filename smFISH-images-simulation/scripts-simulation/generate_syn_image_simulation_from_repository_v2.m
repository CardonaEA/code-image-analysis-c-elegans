function [image_mRNAs_3D_sum, removed_mRNAs_sum] = generate_syn_image_simulation_from_repository_v2(pos_mRNAs, model_image_size, BGD_image, positions_in_oocyte, false_image, concentration, replicate, dim, repository, BGD_max, PSF_substraction)
...
for i = 1 : number_cycles
    ...
    if sum(ismember(out_oocyte, entire_mRNA)) == (dim.Y * dim.X * dim.Z)
        mRNA_image_to_place_wo_BGD = repository{pos_mRNAs.imgs(i)};
        mRNA_image_to_place_wo_BGD(mRNA_image_to_place_wo_BGD < PSF_substraction) = 0;
        mRNA_image_to_place_wo_BGD = mRNA_image_to_place_wo_BGD - BGD_max;
