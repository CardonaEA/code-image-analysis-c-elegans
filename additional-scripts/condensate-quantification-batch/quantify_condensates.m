function quantify_condensates(selected_files, dir_idx, img_idx, par_microscope, crop)
% data info
dataI = struct;
...
disp('getting acquisition info...')
load([dataI.folder dataI.sep dataI.seg_ws], 'CC', 'Cond_IDs', 'number_cells', 'TS_i', 's');

%=== if new microscope parameters are required
if par_microscope.input
    s = par_microscope;
    %s = rmfield(s,'input');
end

%=== fitting average mRNA
disp('fitting average mRNA...')
flags = struct;
...
if ~selected_files{dir_idx}.mRNA_in_folder
    dataI.folder = selected_files{dir_idx}.mRNA_path;
    dataI.mRNA = selected_files{dir_idx}.mRNA_filename;
else
    dataI.mRNA = selected_files{dir_idx}.mRNA_image{1};
end
...
disp('performing condensate quantification...')
flags = struct;
...
disp([' - processing cell number: ' num2str(cell_number_loop) ' of ' num2str(cell_number_total) '  ->  quantifying ' num2str(number_codensates) ' condensates...'])
...
for i = 1: number_codensates
    ...
    [condensates_val, result_TS, integrated_int_TS] = integrated_intensity_batch(flags, s, crop, obj, dataI);
