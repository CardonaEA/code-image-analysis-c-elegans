function [T, T2, Cond_IDs, CC, BP, img_mean, img_median, img_mode] = analysis_granules_by_oocyte_epi_normalization(image_info, parameters, blocks_processing)
%==== load z-stack
image_Data.pathname = image_info.folder;
image_Data.filename = image_info.name;
disp(newline)
disp('reading image...')
[~, image_Data.nIm, raw_image, raw_image_cat] = read_image_epi(image_Data);
disp('...done')

if blocks_processing.is_out_focus
    disp(newline)
    disp('calculating z-boundaries...')
    [blocks_processing] = z_boundaries_det(blocks_processing, image_Data.nIm, raw_image);
    disp('...done')
end
...
%=====  for normalization
disp(newline)
disp('calculating z-stack intensities...')
if blocks_processing.is_out_focus
